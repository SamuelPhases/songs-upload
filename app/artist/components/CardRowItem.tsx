...
const CardRowItem = ({ index, likeLoading, setLikeLoading, setUser, song, user }: Props) => {
    ...
    <div className="flex items-center justify-between gap-3 group hover:bg-gray-900 transition-all rounded-xl p-2 md:px-3 lg:px-5 md:py-3">
      <div className="flex items-center gap-3 flex-1">
          ...
          <button type="button" className="w-5 h-5 group-hover:inline-flex items-center justify-center opacity-0 transition-all group-hover:opacity-100 hidden">
            <Icon icon="line-md:pause-to-play-filled-transition" className="text-lg md:text-xl text-white" />
          </button>
      ...
      <p className="w-[20%] group-hover:text-white text-gray-500 text-[13px] transition-all text-right truncate">{song?.views}</p>
      <div className="flex items-center justify-end gap-5 w-1/4">
          <button type="button" disabled={likeLoading} onClick={()=>handleLike(song?.id)} className="disabled:cursor-not-allowed bg-transparent text-gray-500 hover:text-white focus:text-white active:text-white transition-all md:opacity-0 md:group-hover:opacity-100">
            {likeLoading ? (
              <Icon icon="svg-spinners:eclipse-half" className="text-base md:text-xl" />
            ) : (
              <>{liked ? <Icon icon="line-md:heart-filled" className="text-base md:text-xl text-green-500" /> : <Icon icon="line-md:heart" className="text-base md:text-xl" />}</>
              )}
          </button>
