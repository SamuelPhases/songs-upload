...
const Sidebar = ({user, playlists, setLoading, setLoadingText, updatingProfile, ...props}: SidebarProps) => {
  ...
  return (
    <aside {...props} className={`overflow-hidden hidden md:block transition-all ${sideNav ? 'w-[23%] md:w-[10%] lg:w-[30%] xl:w-1/4 2xl:w-[22%]' : 'w-[23%] md:w-[10%] lg:w-[8%] xl:w-[7%] 2xl:w-[4.5%]'}`}>
        <div className="flex flex-col gap-3 h-full overflow-hidden w-full">
            ...
            <div className="rounded-xl bg-gray-800/40 h-full flex flex-col gap-3 overflow-y-auto px-2 pb-4">
                <div className="flex items-center justify-center lg:justify-between h-fit px-3 py-3">
                    <button
                        type="button"
                    ...
                    {!user ? (
                        ...
                        <div className="grid gap-5">
                            ...
                            <div className="flex flex-col items-center justify-center gap-0.5">
                                {user?.songs?.length > 0 && user && <SidebarLikedSongs user={user} />}
                                {playlists.map((playlist: PlaylistsProps,index) => (
                                    <SidebarMedia key={index} playlist={playlist} updatingProfile={updatingProfile} />
                                ))}
                            </div>
                        </div>
                    )}
